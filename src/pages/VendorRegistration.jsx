...
import SelectField from "../components/ui/SelectField1"; // Import SelectField
...
export default function VendorRegisterPage() {
  ...
  const [businessName, setBusinessName] = useState(""); // Add state for business name
  const [businessCategory, setBusinessCategory] = useState(""); // Add state for business category
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/user/register-vendor", {
        email,
        password,
        businessName,
        businessCategory, // Include business category in the request
  ...
  return (
    <div className="bg-[#FFF8F5]">
      <RegistrationHeader />
      <div
        className="flex items-center justify-center text-black h-[55rem] bg-center bg-no-repeat bg-cover p-28"
