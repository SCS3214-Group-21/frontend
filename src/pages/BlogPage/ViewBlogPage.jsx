...
function ViewBlogPage() {
    ...
    if (blog.length === 0) return <div>No blogs available.</div>; // Handle empty blog list
    ...
    const handleDelete = async () => {
        ...
        const { success, error } = await blogService.deleteBlog(blog_id);
        if (success) {
        ...
        <div>
            ...
            <div className="bg-[#FFF8F5] min-h-screen w-full flex flex-row">
                ...
                <div className="w-[95%] sm:w-[90%] md:w-[80%] px-5 sm:px-10 md:pr-20 md:pl-32 xl:pl-5 xl:pr-16">
                    ...
                    <div className="pb-5">
                        <h1 className='text-4xl font-bold text-custom-primary'>{blog?.title}</h1>
                    </div>
                    <div className="pb-5">
                        <div className='w-full bg-white border border-[#FFDBC8] rounded-xl border-b-8 p-8 flex flex-col'>
                            <h1 className='text-2xl font-semibold text-center text-black'>{blog.title}</h1>
                            <div className="flex flex-row items-end justify-end gap-2">
                                <a href={`/updateblog/${blog.blog_id}`}> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 p-1 text-black rounded-full size-6 hover:bg-custom-secondary hover:text-white">
                                ...
                                <svg
                                    onClick={handleDelete}  // Add onClick to delete blog
                                    xmlns="http://www.w3.org/2000/svg"
                                    fill="none"
                                    viewBox="0 0 24 24"
                                    strokeWidth={1.5}
                                    stroke="currentColor"
                                    className="w-8 h-8 p-1 text-red-700 rounded-full size-6 hover:bg-red-700 hover:text-white"
                                    role="button"
                                >
                                    <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                                </svg>
                            ...
                            <div className="flex items-center justify-center p-5">
                                <img src={blog.img} alt="blog" className="w-full h-full sm:w-3/4 sm:h-3/4" />
                            </div>
                            {/* <div>
                            ...
                            <p className="p-5 text-justify text-black">{blog.description}</p>

                            <h2 className="p-5 font-semibold text-black">Published By : <span className="font-normal">Admin</span></h2>
                            <CommentSection date={formattedDate} time={blog.time} />
                        </div>
                    </div>
                </div>
            </div>
